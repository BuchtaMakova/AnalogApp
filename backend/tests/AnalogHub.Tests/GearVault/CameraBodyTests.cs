...
public sealed class CameraBodyTests
{
    ...
    public async Task Create_ValidRequest_PersistsCameraBodyWithSupportedFormats()
    {
        ...
        var handler = new CreateCameraBodyCommandHandler(db);
        ...
        (await db.CameraBodies.FindAsync(result.Id)).Should().NotBeNull();
    }
    ...
    public async Task Update_ExistingCameraBody_OverwritesAllFieldsAndFormats()
    {
        ...
        var handler = new UpdateCameraBodyCommandHandler(db);
    ...
    {
        using var db = TestDbContextFactory.Create();
        var cameraBody = new CameraBody { Name = "Pentax 645N", Brand = "Pentax", Model = "645N" };
        db.CameraBodies.Add(cameraBody);
        await db.SaveChangesAsync();

        var handler = new DeleteCameraBodyCommandHandler(db);
    ...
    public async Task GetCameraBodies_DefaultsToActiveOnly()
    {
        ...
        db.CameraBodies.AddRange(
            new CameraBody { Name = "Active", Brand = "A", Model = "1", IsActive = true },
            new CameraBody { Name = "Inactive", Brand = "B", Model = "2", IsActive = false });
        await db.SaveChangesAsync();

        var handler = new GetCameraBodiesQueryHandler(db);
    ...
    {
        using var db = TestDbContextFactory.Create();
        var handler = new GetCameraBodyByIdQueryHandler(db);
